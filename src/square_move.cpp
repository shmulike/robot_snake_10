...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "square_move", ros::init_options::NoSigintHandler);
    ros::NodeHandle n;
...
ROS_WARN("Fase : %d\tCounter : %f", fase, counter);
            ...
            if (counter >= 1 && counter < 1.5)
            {
                counter += 2.0*PI/(step*step_factor);
                fase = 4;
            }
           if (counter >= 1.5 && counter < 2)
            {
                counter += 2.0*PI/(step*step_factor);
                fase = 5;
            }
            ...
            if (counter >= 2.25 && counter < 2.5)
            {
                counter += 2.0*PI/(step*step_factor);
                fase = 7;
            }

int arr0, arr1, arr2, arr3, arr4, arr5, arr6, arr7;
            switch(fase){
                case 1:
                ...
                arr[8] = 0;
                arr[9] = 0;
                arr0=sinAmp1 * sin(counter*2*PI);
                arr2=sinAmp1 * sin(counter*2*PI);
                arr4=-sinAmp1 * sin(counter*2*PI);
                arr6=-sinAmp1 * sin(counter*2*PI);
                break;
                ...
                arr[8] =  0;
                arr[9] =  0;
                arr1=sinAmp1 * cos(counter*2*PI);
                arr3=sinAmp1 * cos(counter*2*PI);
                arr5=-sinAmp1 * cos(counter*2*PI);
                arr7=-sinAmp1 * cos(counter*2*PI);
                break;

                case 3:
                arr[0] = -sinAmp1 * cos(counter*2*PI);
                arr[1] ;
                arr[2] = -sinAmp1 * cos(counter*2*PI);
                arr[3] ;
                arr[4] = sinAmp1 * cos(counter*2*PI);
                arr[5] ;
                arr[6] = sinAmp1 * cos(counter*2*PI);
                arr[7] ;
                arr[8] = 0;
                arr[9] = 0;
                arr0=-sinAmp1 * sin(counter*2*PI);
